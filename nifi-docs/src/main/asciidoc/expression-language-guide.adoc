...
//
Telligent Data Pipeline Expression Language Guide
================================================
Telligent Data Team <gene@telligentdata.com>
:homepage: https://www.telligentdata.com

[[overview]]
Overview
--------
All data in the Telligent Data Pipeline is represented by an abstraction called a FlowFile.
...
if the user is unable to make use of them. The TDP Expression Language provides
the ability to reference these attributes, compare them to other values,
and manipulate their values.


[[structure]]
Structure of a TDP Expression
------------------------------

The TDP Expression Language always begins with the start delimiter `${` and ends
...
The Expression Language is used heavily throughout the TDP application for configuring Processor
...
When configuring the value of a Processor property, the TDP User Interface provides help with the
...
*Description*: [.description]#The `replaceAll` function takes two String arguments: a literal String or Regular Expression (TDP uses the Java Pattern
...
*Examples*: We can URL-Encode an attribute named "url" by using the Expression `${url:urlEncode()}`. If
	the value of the "url" attribute is "https://www.telligentdata.com/some value with spaces", this
	Expression will then return "https://www.telligentdata.com/some%20value%20with%20spaces".
...
*Examples*: If we have a URL-Encoded attribute named "url" with the value
	"https://www.telligentdata.com/some%20value%20with%20spaces", then the Expression
	`${url:urlDecode()}` will return "https://www.telligentdata.com/some value with spaces".
...
*Examples*:
	...
	- ${literal(5.4):math("scalb", 2)} runs Math.scalb(5.4, 2). This example is important because TDP EL treats all whole numbers as "longs" and there is no concept of an "int". "scalb" takes a second parameter of an "int" and it is not overloaded to accept longs so it could not be run without special type handling. In the instance where the Java method cannot be found using parameters of type "double" and "long" the "math()" EL function will attempt to find a Java method with the same name but parameters of "double" and "int".
...
*Description*: [.description]#Returns a one-up value (starting at 0) and increasing over the lifetime of the running instance of TDP.
	This value is not persisted across restarts and is not guaranteed to be unique across a cluster.
	This value is considered "one-up" in that if called multiple times across the TDP instance, the values will be sequential.
	However, this counter is shared across all TDP components, so calling this function multiple times from one Processor will
...
`and` and `or` functions. However, this quickly becomes tedious, error-prone, and difficult to maintain. For this reason, TDP
