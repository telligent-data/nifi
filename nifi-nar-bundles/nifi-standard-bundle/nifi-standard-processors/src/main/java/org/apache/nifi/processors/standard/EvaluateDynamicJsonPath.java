...
public class EvaluateDynamicJsonPath extends AbstractJsonPathProcessor {
    ...
    protected PropertyDescriptor getSupportedDynamicPropertyDescriptor(final String propertyDescriptorName) {
        return new PropertyDescriptor.Builder().name(propertyDescriptorName).expressionLanguageSupported(true).addValidator(StandardValidators.NON_EMPTY_VALIDATOR).required(false).dynamic(true).build();
    }
    ...
    public void onTrigger(final ProcessContext processContext, final ProcessSession processSession) throws ProcessException {
        ...
        Set<Map.Entry<String, JsonPath>> attributeJsonPathEntries = processContext.getProperties().entrySet().stream()
                    .filter(e -> e.getKey().isDynamic())
                    .collect(Collectors.toMap(e -> e.getKey().getName(), e -> JsonPath.compile(
                            processContext.getProperty(e.getKey())
                                    .evaluateAttributeExpressions(flowFileCopy)
                                    .getValue())))
                    .entrySet();
